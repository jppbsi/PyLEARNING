...
def main():
    ...
    parser.add_argument('-n','--n_samples', help='number of samples', required=True)
    ...
    plt.show()
    np.savetxt(sys.argv[4], np.c_[Y, np.array(X)], fmt = '%.4f', header = str(n_samples)+' '+str(n_features), comments = '')
